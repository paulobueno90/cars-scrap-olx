...
def frame_atualizar_pesquisa():
    ...
    def window_data():
        ...
        df_size = df_base_model.shape[0]
        print(f"ANUNCIOS: {df_size}")
        pages = (df_size / 5)
        if (df_size / 5) > (df_size // 5):
            pages = int((df_size // 5) + 1)
        ...
        def page_foward():
            ...
            first_shown_ad = int(5 * (current_page - 1))
            last_shown_ad = int(5 * current_page)
            anuncios = int(5 * pages)
            if last_shown_ad > df_size:
                last_shown_ad = df_size

            show_page(current_page, first_shown_ad, last_shown_ad)
        ...
        def show_page(current_page, first_shown_ad, last_shown_ad):

            df_show = df_base_model.iloc[first_shown_ad: last_shown_ad]
            num = 1
            ...
            top_buttons = Frame(window_frame, padx=10, pady=10)
            top_head = Frame(window_frame)

            title = Label(top_head, text=f"ANUNCIOS | Pagina: {current_page} de {int(pages)} paginas.")

            if current_page < pages:
                btn_page_fwd = Button(top_buttons, text="Próxima >>", command=page_foward)
                btn_page_fwd.grid(row=0, column=2)
            if current_page > 1:
                btn_page_bck = Button(top_buttons, text="<< Anterior", command=page_backward)
                btn_page_bck.grid(row=0, column=1)

            title.grid(row=0, column=0)
            top_buttons.grid(row=0, column=1)
            top_head.grid(row=0, column=0)
...
btn2 = Button(frame_buttons, text="Ver Anuncios", command=frame_atualizar_pesquisa, height = 3, width = 17)
